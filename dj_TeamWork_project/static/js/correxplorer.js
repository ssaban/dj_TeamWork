...
//    .style("opacity", 1e-6);
  ...
  var scale = d3.scale.linear()
      ...
      .range([0, parseFloat(d3.select("select#zoom")[0][0].value) * 600]);
  ...
  d3.select("select#zoom").on("change", function() {
  ...
  pixel.enter()
      .append('rect')
          ...
          .on('click', function(d){update_value(d,'elm');});
  ...
  var pixel_mouseover = function(d){
    tooltip.style("opacity", 0.8)
      ...
      .html(d.i + "skill : " + label_row[d.i] + "<br>" + d.j + "Level : " + label_col[d.j] + "<br>" + "Number of Engineers: " + (d.val > 0 ? "+" : "&nbsp;") + d.val.toFixed(3));
  };
